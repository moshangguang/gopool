...
func TestLinklistBlockQueue_PopFrontWithTimeout(t *testing.T) {
	fn := func() {}
	maxLen := 3
	queue := NewLinkListBlockQueue(maxLen)
	for i := 0; i < maxLen; i++ {
		queue.PushBack(fn)
	}
	for i := 0; i < maxLen; i++ {
...
func TestLinkList_PushBack(t *testing.T) {
	fn := func() {}
	maxLen := 3
	queue := NewLinkListBlockQueue(maxLen)
	for i := 0; i < maxLen; i++ {
...
func TestBaseQueue_Close(t *testing.T) {
	fn := func() {}
	maxLen := 3
	queue := NewLinkListBlockQueue(maxLen)
	for i := 0; i < maxLen-1; i++ {
